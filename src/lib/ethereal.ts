...
export async function sendEmailViaNodemailer(to: string, subject: string, html: string) {
  ...
  return { previewUrl: nodemailer.getTestMessageUrl(info) || inboxUrl };
}
...
export async function sendVerificationEmail(to: string, code: string) {
  ...
  `;
  return sendEmailViaNodemailer(to, 'DCamTube: Email Verification', html);
}
